/**
 * Default Settings → Templates values for first-enrollment welcome (single email).
 * Placeholders use {curlyBraces} — substituted at send time.
 *
 * Legacy keys (class_schedule, things_to_prepare, important_reminders, stay_connected)
 * remain in settingsService for backward-compatible DB rows but are no longer sent.
 */

export const FIRST_ENROLLMENT_TEMPLATE_KEYS = Object.freeze({
  onboarding: 'template_first_enrollment_onboarding',
  /** @deprecated No longer sent — kept for Settings DB compatibility */
...
/** Combined welcome email body (all former sequence sections in one message). */
export const COMBINED_ONBOARDING_BODY =
  'Congratulations!\n\n' +
...
export const FIRST_ENROLLMENT_TEMPLATE_DEFAULTS = Object.freeze({
  [FIRST_ENROLLMENT_TEMPLATE_KEYS.onboarding]: {
    ...
    body: COMBINED_ONBOARDING_BODY,
    enabled: true,
  },
  // Legacy section templates (not sent; retained so existing Settings rows still load).
  [FIRST_ENROLLMENT_TEMPLATE_KEYS.class_schedule]: {
    title: 'Your Class Schedule (legacy)',
  ...
  [FIRST_ENROLLMENT_TEMPLATE_KEYS.things_to_prepare]: {
    title: 'Things to Prepare for Class (legacy)',
  ...
  [FIRST_ENROLLMENT_TEMPLATE_KEYS.important_reminders]: {
    title: 'Important Reminders (legacy)',
