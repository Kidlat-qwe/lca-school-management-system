...
| Advance pay skipped ahead of unbilled phases | `POST .../advance-pay` and `POST .../advance-draft` only allow the **next unbilled** local phase; set `generated_count = phase_index`; advance `next_generation_date` / `next_invoice_month` by **one billing cycle** from the current queue (`advanceInstallmentQueueByOneCycle`). `advance-draft` leaves the invoice **Unpaid** for FIUU; enrollment waits until payment. |
