...
- **Multi-phase completed** — `completed` on tracks with more than one phase (prior `new` / `re-enrolled` / `rejoin` **not** required; same as Total Active)
- **Rejoin** matrix cells (`rejoin`)
- **Upsell** matrix cells (`upsell`)

Reserved, dropped, pending enrollment, lifecycle **Inactive (X)**, single-phase completed, and other labels are **inactive** for that month.
