...
- **Active / Inactive (recurring installment)**: on installment tracks, only the **immediate next month/phase** after the latest enrolled or **dropped** billing cell gets payment lifecycle status. When the anchor cell is **dropped**, the next period is always **Inactive** (unpaid next-phase billing). After enrolled cells: **✓ Active** when no unpaid generated invoice exists yet; **X Inactive** when an unpaid invoice is already generated (including under-grace). When the track’s **only** enrollment status is **completed** (single-status / single-phase package), **Active/Inactive is not shown** — completed alone is enough. Once paid, that period is shown from enrollment rows (`new` / `re-enrolled`) and the next immediate period becomes lifecycle-eligible. Drop → later rejoin: intermediate months stay blank (`-`); the real comeback month stays **rejoin** (no synthetic gap rejoins, and no rename to re-enrolled).
