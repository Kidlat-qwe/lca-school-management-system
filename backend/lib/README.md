...
- Month/phase matrix **rate header** numerator = visible **`re-enrolled`** + lifecycle **Active (✓)**. **Completed** (including standalone) is **excluded** from the rate (use the Completed KPI / `kpi_totals.completed_count`). **Inactive (X)** is excluded. **Denominator** = prior month/phase cells labeled `new`, `re-enrolled`, `rejoin`, or `upsell`, plus `completed` only when that prior completed cell already had an earlier new/re-enrolled/rejoin on the same track.
...
- **Upsell** (e.g. Pre-K → KG): month matrix merges the higher program onto the lower-program **same row**. Display starts at the **later** of (month after lower completion) and the higher track’s first billing month — so a gap month stays empty when the student starts the higher program later (e.g. Nursery completed April, Pre-K starts June → May `-`, June upsell). Each higher-program billing phase maps to consecutive month columns (re-enrolled, then completed on the terminal phase). The higher-class row is hidden. Phase indexing uses full billing metadata so **later calendar years** show continuation on that same row (e.g. Jan–Mar 2027). Requires a higher `level_tag` on the sibling class track.
...
- **Installment plan enrollment column**: the first **paid** phase on the plan shows **new** even when `phase_start` is 1 and billing begins at a later absolute phase (e.g. Phase 2 after a skipped late-start gap). **One-phase** plans (`total_phases = 1`) show **completed** once paid.
- **Single-phase completion**: one-phase classes and one-phase installment plans mark the sole enrolled month/phase as **completed** (not **new**) once paid / package complete.
- **Active / Inactive (full payment)**: on multi-phase full-payment tracks, when the latest enrolled cell is **completed**, the immediate next empty month/phase is **Inactive (X)** (e.g. July `completed` → August `Inactive`). Single-status short courses still show completed only. Repair example: `backend/scripts/repairBriaToledanoFullPaymentThroughJuly.js`.
- **Active / Inactive (recurring installment)**: on installment tracks, only the **immediate next month/phase** after the latest enrolled or **dropped** billing cell gets payment lifecycle status. When the anchor cell is **dropped**, the next period is always **Inactive** (unpaid next-phase billing). After enrolled cells: **✓ Active** while nothing is overdue — no unpaid generated invoice yet, **or** the generated invoice is unpaid but its **due date has not passed** (Manila calendar); **X Inactive** only once that due date is already past and still unpaid (e.g. Phase 1 paid in July + Phase 2 generated Jul 25 due Aug 05 → August stays **Active** until Aug 06). An explicitly inactive installment profile also forces the immediate next cell to **Inactive**, preventing a finished/stopped plan with no generated next invoice from remaining falsely Active. Unpaid invoices without `TARGET_PHASE` remarks still map to billing months via `ROW_NUMBER` by `issue_date` (same fallback as the paid-phase gap overlay) so overdue plans do not stick on Active with a null due date. When the track’s **only** enrollment status is **completed** (single-status short course), **Active/Inactive is not shown** — completed alone is enough. For multi-phase / recurring installment tracks, when the latest matrix anchor is **completed** (or the installment package is already complete), the immediate next month/phase is always **Inactive (X)** — never **Active** after recurring invoices are finished (e.g. April `completed` → May `Inactive`; that student counts inactive for May). Once paid, that period is shown from enrollment rows (`new` / `re-enrolled`) and the next immediate period becomes lifecycle-eligible. Drop → later rejoin: intermediate months stay blank (`-`); the real comeback month stays **rejoin** (no synthetic gap rejoins, and no rename to re-enrolled).
- **Cross-class installment display**: when billing lives on the profile class but enrollment was auto-created on a different class (same student), the display-class row sets `matrix_installment_class_id`, copies billing-month cells from the profile class, strips wrong display-only enrolled months, and hides the profile-class row. Drop/rejoin gap fill is skipped on those rows so lifecycle can show **Inactive** after **dropped**. Repair script: `backend/scripts/repairKievZionSerranoMatrixAlignment.js` (Kiev Zion Z. Serrano reference case).
- **Soft-removed enrollments**: if `removed_at` falls in the same billing month or earlier (Asia/Manila), that month’s enrolled badge (`new` / `re-enrolled` / …) is cleared so it does not inflate Month Re-enrollment KPIs or Total Active Students (e.g. unenrolled mid-July after a wrong-class `new`). Months before the removal keep their historical badges.
- **Partial / open balance installment**: `syncInstallmentEnrollmentForPaidInvoice` creates `new` / `re_enrolled` only when the phase chain is **fully settled**. Partial advance/payment keeps Enrollment blank so the month matrix can show lifecycle **Inactive** and Month Re-enrollment / Monthly Operational Total Active / Student Status stay aligned (no premature Active from unpaid remaining balance).
- **Lifecycle Inactive cells** never count toward Total Active / Student Status (`isMonthMatrixCellActiveForOperationalDashboard`).
- **Paid installment phase gaps**: when a paid installment phase has no matching `classstudentstbl` row (e.g. phase 2 paid but only phases 1 and 3 enrolled), the month matrix synthesizes **re-enrolled** on that phase’s billing month from paid invoice order (`ROW_NUMBER` by `issue_date`, excluding downpayment). Phase numbers fall back to invoice sequence when `TARGET_PHASE` remarks are absent. Does **not** synthesize over phases that already have a **dropped**/removed `classstudentstbl` row (keeps matrix aligned with invoice Enrollment column).
...
**Partial downpayment + balance invoice:** `createBalanceInvoiceFromPartial` must **not** rewrite `downpayment_invoice_id` to the balance leaf — keep the chain root. Student History phases API matches DP by chain root **or** any invoice on that chain, and shows DP status from **chain settlement** (Paid when remaining = 0), not the partial parent row alone. Settling the DP balance marks `downpayment_paid` when the DP chain is fully settled; it does **not** mark Phase 1 paid (`downpayment_only`).
