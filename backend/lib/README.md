...
- Month/phase matrix **rate header** numerator = visible purple **`re-enrolled`** cells only (same as Re-enrollment KPI / matrix badge count). **Completed** is not in the numerator (shown on the Completed card). **Denominator** = prior month/phase cells labeled `new`, `re-enrolled`, `rejoin`, or `upsell` only.
...
- **Total Active Students** = new + re-enrolled + rejoin + upsell + multi-phase completed (single-phase completed e.g. Active Champs excluded).
