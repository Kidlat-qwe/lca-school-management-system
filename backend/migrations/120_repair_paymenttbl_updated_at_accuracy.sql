-- Repair paymenttbl.updated_at accuracy (run after 119 if ADD COLUMN DEFAULT stamped migration time).

ALTER TABLE public.paymenttbl
  ADD COLUMN IF NOT EXISTS updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP;

-- Drop triggers BEFORE backfill so UPDATE does not overwrite with CURRENT_TIMESTAMP.
...
-- Backfill: last known system event on the row (create / approve / return / reject).
