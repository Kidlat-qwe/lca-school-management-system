/**
 * Mariae Erica Maere F. Fernando (gewellmaefernando@gmail.com) — move mistaken payment.
 *
 * Wrong: Plan 1 (profile 124 / VMP_Nursery_TThS_9:30AM) Phase 7 INV-843
 *        paid with PAY-733 (₱4,236, AR 260500) on 2026-05-07.
 * Correct: Plan 2 (profile 414 / VMP_NURSERY_TThS_11:00 AM) Phase 3 INV-1346
 *          should be Paid with that payment; Plan 1 Phase 7 should be dropped.
 *
 * Notes:
 *   - INV-1346 is Unpaid with late penalty (₱4,659.60 = ₱4,236 + ₱423.60).
 *     Repair waives the penalty so settlement matches PAY-733.
 *   - PAY-1127 Rejected (“double encode”) stays for audit.
 *   - INV-1346 remarks lack TARGET_PHASE; repair appends TARGET_PHASE:3.
 *   - Phase 3 enrollment is updated by a follow-up enrollment script.
 *   - This student exists on **production** (`psms_production`), not development.
 *
 * Run:
 *   node backend/scripts/repairEricaFernandoMovePhase7PaymentToPlan2Phase3.js --production
 *   node backend/scripts/repairEricaFernandoMovePhase7PaymentToPlan2Phase3.js --production --apply
 */
...
const STUDENT_EMAIL = 'gewellmaefernando@gmail.com';
const STUDENT_ID = 269;

/** Plan 1 (9:30AM) — wrong payment target (drop Phase 7 after move) */
const PLAN1_PROFILE_ID = 124;
const PLAN1_CLASS_ID = 63;
const SOURCE_INVOICE_ID = 843; // Phase 7
const SOURCE_PHASE = 7;
const SOURCE_ENROLLMENT_ID = 745;

/** Plan 2 (11:00AM) — correct payment target */
const PLAN2_PROFILE_ID = 414;
const TARGET_INVOICE_ID = 1346; // Phase 3 (phase_start of Plan 2)
const TARGET_PHASE = 3;
const TARGET_EXISTING_AR = '261009'; // clear before stamping payment AR

const PAYMENT_ID = 733;
const PAYMENT_AMOUNT = 4236.0;
const PAYMENT_AR_NUMBER = '260500';
const PAYMENT_ISSUE_DATE = '2026-05-07';

const REJECTED_PAYMENT_ID = 1127; // stays Rejected on INV-1346

const REPAIR_NOTE =
  'Ops repair 2026-08-01 — Erica Fernando move PAY-733 from Plan1 Phase7 INV-843 → Plan2 Phase3 INV-1346; drop Plan1 Phase7';const isApply = process.argv.includes('--apply');
...
async function main() {
  console.log(
    `\nErica Fernando — move Plan1 Phase7 payment → Plan2 Phase3` +
  ...
  try {
    ...
    console.log('\nPlanned changes:');
    console.log(
      `  0. Waive late penalty on INV-${TARGET_INVOICE_ID} → base ₱${PAYMENT_AMOUNT}`
    );
    console.log(
