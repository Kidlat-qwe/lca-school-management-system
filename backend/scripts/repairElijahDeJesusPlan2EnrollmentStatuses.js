/**
 * Elijah Mikael A. De Jesus (aquinomarielle221@gmail.com) — align Plan 2 (11:00AM Nursery)
 * enrollment statuses for Student History + Month/Phase Re-enrollment matrix.
 *
 * Target (class 53 / profile 412):
 *   Phase 3 → new
 *   Phase 4 → re_enrolled
 *   Phase 5 → re_enrolled
 *
 * Run:
 *   node backend/scripts/repairElijahDeJesusPlan2EnrollmentStatuses.js --production
 *   node backend/scripts/repairElijahDeJesusPlan2EnrollmentStatuses.js --production --apply
 */
...
const REPAIR_NOTE =
  'Ops repair 2026-08-01 — Elijah De Jesus Plan2 enrollment: Phase3 new, Phase4–5 re_enrolled';
...
const PHASES = [
  ...
  },
  {
    ...
    phase: 5,
    status: 're_enrolled',
    enrolledAt: '2026-07-05 12:00:00+08',
    clearRemoved: true,
  },
];
...
async function main() {
  console.log(
    `\nElijah De Jesus — Plan2 enrollment statuses` +
  ...
  try {
    ...
    console.log('\nBEFORE month matrix (Elijah track):');
    console.table(beforeMatrix.monthCells);
    console.log('BEFORE phase matrix (Elijah track):');
    ...
    console.log('\nAFTER month matrix (Elijah track):');
    console.table(afterMatrix.monthCells);
    console.log('AFTER phase matrix (Elijah track):');
