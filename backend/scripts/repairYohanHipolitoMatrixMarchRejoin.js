/**
 * Yohan Teodoro Hipolito (student 413) — month re-enrollment matrix alignment.
 *
 * Issues:
 *   - Phase 1 "new" appears in April; student paid / first billing is March.
 *   - Phase 2 incorrectly stored as "new" (should be re_enrolled).
 *   - Phase 5 after delinquency drop stored as re_enrolled (should be rejoin).
 *
 * Class: 67 VMP_Playgroup_TTh_11:00AM | Profile: 160 | Branch: LCA Pampanga
 *
 * Expected matrix after repair:
 *   Mar new → Apr/May re-enrolled → Jun dropped → Jul rejoin → Aug Active
 *
 * Run:
 *   node backend/scripts/repairYohanHipolitoMatrixMarchRejoin.js
 *   node backend/scripts/repairYohanHipolitoMatrixMarchRejoin.js --apply
 */
...
const PHASE1_CLASSSTUDENT_ID = 369;
const PHASE2_CLASSSTUDENT_ID = 698;
const PHASE5_CLASSSTUDENT_ID = 1581;
const MARCH_ENROLLED_AT = '2026-03-24T00:00:00+08:00';
...
async function main() {
  console.log(
    `\nYohan Hipolito — matrix March new + rejoin${isApply ? ' (APPLY)' : ' (DRY RUN)'}\n`
  );
  ...
  try {
    ...
    console.log('\nPlanned changes:');
    console.log(
      `  • UPDATE classstudent ${PHASE1_CLASSSTUDENT_ID} enrolled_at → 2026-03-24 (Manila)`
    );
    console.log(
    ...
    console.table([
      ...
      { month: '2026-05', label: 're-enrolled', phase: 3 },
    ...
    console.log('\n✅ Applied Yohan Hipolito matrix alignment.');
