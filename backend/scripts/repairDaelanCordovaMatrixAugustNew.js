/**
 * Daelan Alaistair Shun Cordova — month matrix: Phase 2 "new" in August.
 *
 * Student: 673 · gwenthampal14@gmail.com
 * Class: 121 SOMO_JULY_Pre-Kinder_MWF_2:30 PM · Profile: 498 · Branch: VMC (3)
 * Phase 2 classstudent: 1955 · INV-2366 (TARGET_PHASE:2)
 *
 * Current: enrolled_at 2026-07-28 → July new / August Active
 * Expected: August new → September Active
 *
 * Run:
 *   node backend/scripts/repairDaelanCordovaMatrixAugustNew.js --production
 *   node backend/scripts/repairDaelanCordovaMatrixAugustNew.js --production --apply
 */
...
const CLASS_ID = 121;
const BRANCH_ID = 3;
const PROFILE_ID = 498;
const PHASE2_CLASSSTUDENT_ID = 1955;
const PHASE2_INVOICE_ID = 2366;
...
const REPAIR_NOTE =
  'Ops repair 2026-08-07 — Daelan Cordova matrix new month July→August (phase 2 enrolled_at + due)';
...
async function main() {
  console.log(
    `\nDaelan Cordova — matrix August new${isApply ? ' (APPLY)' : ' (DRY RUN)'}\n`
  );
  ...
  try {
    ...
    if (!enroll || Number(enroll.phase_number) !== 2) {
      throw new Error(
        `Phase 2 enrollment ${PHASE2_CLASSSTUDENT_ID} not found (got ${JSON.stringify(enroll)})`
      );
    }
    ...
    if (!String(inv.remarks || '').includes('TARGET_PHASE:2')) {
      throw new Error(`INV-${PHASE2_INVOICE_ID} is not TARGET_PHASE:2`);
    }
    ...
    console.log('\nPlanned:');
    console.log(
      `  1. classstudent ${PHASE2_CLASSSTUDENT_ID} enrolled_at → ${AUG_ENROLLED_AT}`
    );
    console.log(
      `  2. INV-${PHASE2_INVOICE_ID} issue/due → ${PHASE2_ISSUE} / ${PHASE2_DUE}`
    );
    ...
    await client.query(
      `UPDATE classstudentstbl
       ...
       WHERE classstudent_id = $3
         AND student_id = $4
         AND class_id = $5`,
      [AUG_ENROLLED_AT, REPAIR_NOTE, PHASE2_CLASSSTUDENT_ID, STUDENT_ID, CLASS_ID]
    );
    ...
    await client.query(
      ...
      [PHASE2_ISSUE, PHASE2_DUE, PHASE2_INVOICE_ID, REPAIR_NOTE, PROFILE_ID]
    );
    await syncProgramPaymentStatusForInvoice(client, PHASE2_INVOICE_ID);
    console.log(`✅ INV-${PHASE2_INVOICE_ID} → ${PHASE2_ISSUE} / ${PHASE2_DUE}`);
