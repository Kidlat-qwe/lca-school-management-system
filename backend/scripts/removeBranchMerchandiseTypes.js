/**
 * Remove one or more merchandise type cards (and their stock rows) for one branch.
 *
 * Exact name match (case-insensitive trim). Does not touch other branches or types.
 *
 * FK cleanup before DELETE (same pattern as clearAllMerchandise):
 ...
 *
 * Required:
 *   --branch-name=Malolos|Cavite|Guiguinto|…
 *   --names="Name One,Name Two"   comma-separated exact CMS merchandise_name values
 *
 * Optional:
 *   --branch-id=1   force id (must still match --branch-name)
 *
...
const branchNameFragment = String(argValue('branch-name') || '').trim();
const namesRaw = String(argValue('names') || '').trim();
...
if (!branchNameFragment) {
  console.error('Required: --branch-name=Malolos (or Cavite, Guiguinto, …)');
  process.exit(1);
}

if (!namesRaw) {
  console.error(
    'Required: --names="Recognition Program,Toga Set" (comma-separated exact merchandise names)'
  );
  process.exit(1);
}

const targetNames = namesRaw
  .split(',')
  .map((n) => n.trim())
  .filter(Boolean);

if (targetNames.length === 0) {
  console.error('No merchandise names parsed from --names=');
  process.exit(1);
}
...
async function main() {
  ...
  try {
    ...
    const nameKeys = targetNames.map((n) => normalizeName(n));
    ...
    const foundKeys = new Set(rows.map((r) => normalizeName(r.merchandise_name)));
    const missing = targetNames.filter((n) => !foundKeys.has(normalizeName(n)));
    if (missing.length) {
      console.log(`Names with no rows on this branch: ${missing.map((n) => `"${n}"`).join(', ')}`);
    }

    if (rows.length === 0) {
      console.log(`\nNo matching merchandise rows on ${branchNameFragment}. Nothing to do.`);
      return;
    }
    ...
    if (isDryRun) {
      ...
      console.log(
        `  node scripts/removeBranchMerchandiseTypes.js --production --branch-name=${branchNameFragment} --names="${targetNames.join(',')}" --apply`
      );
      return;
    }
    ...
    console.log(
      `\nRefresh Merchandise for ${branchNameFragment} — removed: ${targetNames.map((n) => `"${n}"`).join(', ')}.`
    );
  } catch (err) {
    try {
    ...
    console.error('\nremoveBranchMerchandiseTypes failed:', err.message);
    process.exitCode = 1;
  } finally {
    client.release();
  }
