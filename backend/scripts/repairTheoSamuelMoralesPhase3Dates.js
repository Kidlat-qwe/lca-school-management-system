/**
 * Theo Samuel P. Morales (Pre-Kindergarten Installment Plan 3) —
 * fix Phase 3 issue/due dates + confirm installment queue dates.
 *
 * Student history shows Phase 3 (INV-1761 / AR 261425) as:
 *   issue June 25, due July 05 → Overdue
 * (same issue date as paid Phase 2 INV-1760 — wrong cadence)
 *
 * Ops-correct:
 *   Phase 3 — INV-1761: issue 2026-07-25, due 2026-08-05
 *   Installment Invoice Logs queue:
 *     next_generation_date 2026-08-25
 *     next_invoice_month   2026-09-01
 *
 * Email (DB): charlsmorales01@gmail.com
 * Profile: 371 · class VMM_Pre-Kinder_MWF 11:00 AM
 *
 * Run (from backend/):
...
const STUDENT_EMAIL = 'charlsmorales01@gmail.com';
const STUDENT_ID = 304;
const PROFILE_ID = 371;

/** Phase 3 invoice only */
const PHASE_TARGETS = {
  1761: { absolute_phase: 3, issue_date: '2026-07-25', due_date: '2026-08-05' },
};
...
const REPAIR_NOTE =
  'Ops repair — Theo Samuel Morales Phase 3 issue/due Jul 25 / Aug 05 + queue Aug 25 / Sep 01';
...
async function main() {
  console.log(
    `\nTheo Samuel Morales — Phase 3 dates + queue${
  ...
  try {
    ...
    const verifyInv = await client.query(
      `SELECT invoice_id, status, invoice_ar_number,
              ...
              amount
       FROM invoicestbl
       WHERE invoice_id = 1761`
    );
    console.log('\nVerified Phase 3 invoice:');
