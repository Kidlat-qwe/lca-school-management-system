/**
 * Asher Benedict Perez — fix Phase 9 issue/due dates and clear penalty.
 *
 * Student email: e.guintivano@gmail.com
 * Invoice: INV-2357 (TARGET_PHASE:9) · AR 262022
 *
 * Before (from Student History screenshot):
 *   Issued 2026-07-27 · Due 2026-07-20 · Unpaid (due before issue)
 *
 * Target:
 *   issue_date 2026-08-25
 *   due_date   2026-09-05
 *   clear any late penalty line items / late_penalty_applied_for_due_date
 *   installment queue scheduled_date → 2026-09-05 (when queue row exists)
 *
 * Run (production DB):
 *   node backend/scripts/repairAsherPerezPhase9Dates.js --production
 *   node backend/scripts/repairAsherPerezPhase9Dates.js --production --apply
 */
...
const STUDENT_EMAIL = 'e.guintivano@gmail.com';
const PHASE9_INVOICE_ID = 2357;
...
const REPAIR_NOTE =
  'Ops repair 2026-09-01 — Asher Perez Phase 9 dates Aug 25 / Sep 5, clear penalty';
...
async function loadInvoice(client, invoiceId) {
  const r = await client.query(
    `SELECT i.invoice_id, i.status, i.remarks, i.amount,
...
async function clearInvoicePenalty(client, invoiceId) {
  const items = await client.query(
    `SELECT invoice_item_id, amount, penalty_amount, description
     FROM invoiceitemstbl
     WHERE invoice_id = $1
       AND (
         COALESCE(penalty_amount, 0) > 0
         OR LOWER(COALESCE(description, '')) LIKE '%penalty%'
         OR LOWER(COALESCE(description, '')) LIKE '%late%'
       )`,
    [invoiceId]
  );
  ...
  for (const item of items.rows) {
    const hasPenaltyAmt = Number(item.penalty_amount || 0) > 0;
    const looksPenalty =
      /penalty|late/i.test(String(item.description || '')) &&
      Number(item.amount || 0) > 0;
    if (!hasPenaltyAmt && !looksPenalty) continue;
  ...
  return { clearedLines, amount: grand };
}

async function main() {
  console.log(
    `\nAsher Benedict Perez — Phase 9 (INV-${PHASE9_INVOICE_ID}) → ${TARGET_ISSUE} / ${TARGET_DUE}` +
      ` + clear penalty${isApply ? ' (APPLY)' : ' (DRY RUN)'}\n`
  );
  ...
  try {
    ...
    console.log(`INV-${PHASE9_INVOICE_ID} BEFORE:`, {
      ...
      amount: inv.amount,
      phase: inv.phase,
      late_penalty_applied_for_due_date: inv.late_penalty_applied_for_due_date,
    });
    ...
    const penaltyResult = await clearInvoicePenalty(client, PHASE9_INVOICE_ID);
    console.log(
      penaltyResult.clearedLines > 0
        ? `✅ Cleared ${penaltyResult.clearedLines} penalty line(s); invoice amount → ${penaltyResult.amount}`
        : `✅ No penalty lines found; late_penalty flag cleared; amount → ${penaltyResult.amount}`
    );
    ...
    console.log(`INV-${PHASE9_INVOICE_ID} AFTER:`, {
      ...
      amount: invAfter.amount,
      late_penalty_applied_for_due_date: invAfter.late_penalty_applied_for_due_date,
    });
    ...
    console.log('\nExpected UI Phase 9: Issued Aug 25, 2026 · Due Sep 5, 2026 · no penalty');
