/**
 * Alonzo Xavier De Luna (Playgroup Installment Plan 1) —
 * fix Phase 7 issue/due dates + confirm installment queue dates.
 *
 * Student history shows Phase 7 (INV-2347 / AR 262012) as:
 *   issue July 26, due August 05 (same day as paid Phase 6 — wrong cadence)
 *
 * Ops-correct:
 *   Phase 7 — INV-2347: issue 2026-08-25, due 2026-09-05
 *   Installment Invoice Logs queue:
 *     next_generation_date 2026-09-25
 *     next_invoice_month   2026-10-01
 *
 * Email (DB): larainerabago@gmail.com
 * Profile: 495 · class SOMO_Playgroup_SS_11:00-12:00PM · phase_start 6
 *
 * Run (from backend/):
...
/** Phase 7 invoice only */
const PHASE_TARGETS = {
  2347: { absolute_phase: 7, issue_date: '2026-08-25', due_date: '2026-09-05' },
};
...
const REPAIR_NOTE =
  'Ops repair — Alonzo De Luna Phase 7 issue/due Aug 25 / Sep 05 + queue Sep 25 / Oct 01';
...
async function main() {
  console.log(
    `\nAlonzo De Luna — Phase 7 dates + queue${
  ...
  try {
    ...
    const verifyInv = await client.query(
      `SELECT invoice_id, status, invoice_ar_number,
              ...
              amount
       FROM invoicestbl
       WHERE invoice_id = 2347`
    );
    console.log('\nVerified Phase 7 invoice:');
