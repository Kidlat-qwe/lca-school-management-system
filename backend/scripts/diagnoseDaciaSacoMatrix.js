/**
 * Diagnose Dacia Candice L. Saco month matrix labels.
 */
import '../config/loadEnv.js';
import { query } from '../config/database.js';
import { loadStudentMonthEnrollmentMatrix } from '../lib/enrollmentRateMetrics.js';

const name = '%Dacia%Saco%';
const email = 'ligasongemerlyn@gmail.com';

const userRes = await query(
  `SELECT user_id, full_name, email FROM userstbl
   WHERE (full_name ILIKE $1 OR email ILIKE $2) AND user_type = 'Student'
   LIMIT 5`,
  [name, email]
);
...
const payments = await query(
  `SELECT p.payment_id, p.invoice_id, p.payable_amount, p.status, p.approval_status, p.created_at,
          TO_CHAR(TIMEZONE('Asia/Manila', p.created_at), 'YYYY-MM-DD') AS pay_manila,
...
console.log('\n=== All payments ===');
for (const r of payments.rows) console.log(r);
