/**
 * Zion Alexander Infante De Leon — correct installment invoice issue/due dates
 * for phases 8–10 (profile 116, VMP_Nursery_MWF_9:30AM).
 *
 *   Phase 8 — issue 2026-04-25, due 2026-05-05  (INV-915)
 *   Phase 9 — issue 2026-05-25, due 2026-06-05  (INV-1415)
 *   Phase 10 — issue 2026-06-25, due 2026-07-05 (create if missing)
 *
 * Run:
 *   node backend/scripts/repairZionDeLeonInstallmentPhases8To10.js
 *   node backend/scripts/repairZionDeLeonInstallmentPhases8To10.js --apply
 */
...
const STUDENT_EMAIL = 'deleonalexyo@gmail.com';
const STUDENT_ID = 263;
const PROFILE_ID = 116;
const CLASS_ID = 61;
const PHASE_AMOUNT = '4999.00';
const REPAIR_NOTE = 'Ops repair 2026-07-07 — Zion De Leon phases 8–10 issue/due dates';

const PHASE_8_INVOICE_ID = 915;
const PHASE_9_INVOICE_ID = 1415;
const TEMPLATE_INVOICE_ID = 1415;
...
async function main() {
  console.log(
    `\nZion De Leon — phases 8–10 issue/due repair${isApply ? ' (APPLY)' : ' (DRY RUN)'}\n`
  );
  ...
  try {
    ...
    for (const [invoiceIdStr, target] of Object.entries(PHASE_TARGETS)) {
      ...
      if (curIssue !== target.issue_date || curDue !== target.due_date || curTp !== target.absolute_phase) {
        changes.push({
          action: 'update_dates',
    ...
    for (const [invoiceIdStr, target] of Object.entries(PHASE_TARGETS)) {
      const invoiceId = Number(invoiceIdStr);
      const inv = (
        await client.query(`SELECT status FROM invoicestbl WHERE invoice_id = $1`, [invoiceId])
      ).rows[0];
    ...
    console.log('\n✅ Done. Refresh Student History → Invoices for Zion De Leon.');
  } catch (err) {
