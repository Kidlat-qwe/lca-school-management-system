/**
 * Kier Carlo Cruz Balagtas (it.kier@little-champion.com) — generate Phase 2.
 *
 * Development profile #254 · Class 56 MID_PHASES · Branch 1 · Student 38
 * Phase 1 paid (invoice 724 / AR 260583, issue 2026-09-04).
 * Queue already at Nov 1 / Nov 1 (1_5 cadence after late-Sep class start).
 * Phase 2 is not due until 2026-11-01; this script force-generates it for ops/testing.
 *
 * Expected after --apply --generate:
 *  - Phase 2 unpaid invoice issue 2026-11-01 / due 2026-11-05
 *  - Queue advances to Dec 1 / Dec 1
 *  - generated_count = 2
 *
 * Run (from backend/):
 *   node scripts/repairKierBalagtasGeneratePhase2.js
 *   node scripts/repairKierBalagtasGeneratePhase2.js --apply --generate
 *   node scripts/repairKierBalagtasGeneratePhase2.js --production --apply --generate
 */
...
const STUDENT_ID = 38;
const PROFILE_ID = 254;
const CLASS_ID = 56;
const TARGET_PHASE = 2;
const TARGET_GENERATION_DATE = '2026-11-01';
const TARGET_INVOICE_MONTH = '2026-11-01';
const AFTER_GENERATE_NEXT_GEN = '2026-12-01';
const AFTER_GENERATE_NEXT_MONTH = '2026-12-01';
const EXPECTED_ISSUE = '2026-11-01';
const EXPECTED_DUE = '2026-11-05';

const REPAIR_NOTE =
  'Ops — Kier Balagtas (it.kier@little-champion.com) generate Phase 2 (Nov 1 / Nov 5)';
...
function profileForSchedule(row) {
  return {
    ...
    // Required so cadence inference uses the 1_5 Nov-1 queue (not default 25_5).
...
async function main() {
  console.log(
    `\nKier Balagtas — Phase 2 generate${isApply ? ' (APPLY)' : ' (DRY RUN)'}${
      isGenerate ? ' + GENERATE' : ''
    }\n`
  );
  ...
  try {
    ...
    if (needsQueue) {
      ...
      console.log('  1. Queue already at Nov 1 / Nov 1');
    } else {
    ...
    if (!phase2Before) {
      ...
      console.log('  2. Phase 2 already present — ensure dates + queue');
    }
