 ...
 *
 * Wrong: Plan 1 (profile 128 / VMP_Nursery_TThS_9:30AM) Phase 7 INV-794
 *        paid with PAY-671 (₱5,146, AR 260451) on 2026-05-05.
 * Correct: Plan 2 (profile 413 / VMP_NURSERY_TThS_11:00 AM) Phase 3 INV-1344
 *          should be Paid with that payment; Plan 1 Phase 7 should be dropped.
 *
 * Notes:
 *   - INV-1344 is currently Rejected (PAY-1125 Rejected — "Double encode").
 *     That rejected payment stays for audit; PAY-671 becomes the settling payment.
 *   - INV-1344 remarks lack TARGET_PHASE; repair appends TARGET_PHASE:3.
 *   - Phase 3 enrollment (classstudent 1182) stays dropped historically
 *     (student later rejoined at Phase 4+); we do not re-insert Phase 3 enrollment.
 *   - This student exists on **production** (`psms_production`), not development.
 *
 * Run:
 *   node backend/scripts/repairOliviaSalesMovePhase7PaymentToPlan2Phase3.js --production
 *   node backend/scripts/repairOliviaSalesMovePhase7PaymentToPlan2Phase3.js --production --apply
 */
...
const SOURCE_INVOICE_ID = 794; // Phase 7
const SOURCE_PHASE = 7;
const SOURCE_ENROLLMENT_ID = 693;

/** Plan 2 (11:00AM) — correct payment target */
const PLAN2_PROFILE_ID = 413;
const TARGET_INVOICE_ID = 1344; // Phase 3 (phase_start of Plan 2)
const TARGET_PHASE = 3;
const TARGET_EXISTING_AR = '261007'; // clear before stamping payment AR

const PAYMENT_ID = 671;
const PAYMENT_AMOUNT = 5146.0;
const PAYMENT_AR_NUMBER = '260451';
const PAYMENT_ISSUE_DATE = '2026-05-05';

const REJECTED_PAYMENT_ID = 1125; // stays Rejected on INV-1344

const REPAIR_NOTE =
  'Ops repair 2026-08-01 — Olivia Sales move PAY-671 from Plan1 Phase7 INV-794 → Plan2 Phase3 INV-1344; drop Plan1 Phase7';
...
async function main() {
  console.log(
    `\nOlivia Sales — move Plan1 Phase7 payment → Plan2 Phase3` +
