...
import { createBalanceInvoiceFromPartial, getChainFinancialSummary } from '../utils/balanceInvoice.js';
import {
  isPenaltyExemptByPlanPosition,
} from '../utils/installmentPenaltyExempt.js';
import {
...
import { evaluateClassInstallmentBillingEligibility } from '../utils/billingNotificationEligibility.js';
import { resolveInstallmentPlanLifecycleActive } from '../utils/installmentPlanLifecycleStatus/index.js';
import {
...
import {
  listUpcomingDelinquencyDrops,
  syncInstallmentDelinquencyDropsForProfile,
...
/**
 * GET /installment-invoices/upcoming-delinquency-drops
 *
 * Students whose unpaid installment phase will auto-drop within 7 days.
 * - Admin: own branch only (powers login modal + Installment Invoice tab)
 * - Superadmin: all branches, or optional ?branch_id= (tab only; no login modal)
 */
router.get(
  '/upcoming-delinquency-drops',
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (userType === 'Superadmin') {
        ...
        branchId =
          qBranch != null && String(qBranch).trim() !== '' && Number.isFinite(Number(qBranch))
            ? Number(qBranch)
            : null;
      } else if (!branchId) {
        return res.status(403).json({
          success: false,
          message: 'Only branch Admin users have an upcoming-delinquency-drops list.',
        });
      }

      const data = await listUpcomingDelinquencyDrops(client, {
        branchId,
        withinDays: 7,
      });
      ...
      if (branchId) {
        try {
          const branchRes = await client.query(
            `SELECT branch_name, branch_nickname FROM branchestbl WHERE branch_id = $1`,
            [branchId]
          );
          const b = branchRes.rows[0];
          branchName = b?.branch_nickname || b?.branch_name || null;
        } catch {
          /* ignore */
        }
      }
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const delinquencyClient = await getClient();
      try {
        await delinquencyClient.query('BEGIN');
        await syncInstallmentDelinquencyDropsForProfile(delinquencyClient, Number(id));
        await delinquencyClient.query('COMMIT');
      } catch (dropSyncErr) {
        await delinquencyClient.query('ROLLBACK').catch(() => {});
        console.error(`Delinquency drop sync failed for profile ${id}:`, dropSyncErr);
      } finally {
        delinquencyClient.release();
      }
      ...
      const computeStatus = (invoiceStatus, dueDate, issueDate, penaltyExempt = false) =>
        computeInstallmentPhaseDisplayStatus({
          invoiceStatus,
          dueDateYmd: dueDate,
      ...
      const buildPhaseRow = async (phaseNumber, chain) => {
        ...
        const chainSummary = await getChainFinancialSummary(pool, chain.chain_root_id);
        const totalObligation = Number(chainSummary.total_obligation || 0);
        const paidAmount = Number(chainSummary.total_paid_in_chain || 0);
        const remainingBalance = Math.max(0, Number(chainSummary.remaining_on_leaf || 0));
        const payableInvoiceId = Number(chainSummary.payable_invoice_id);
        const rootInvoiceId = Number(chainSummary.root_invoice_id);

        const isAdvance = isAdvancePaymentInvoice(rep) || isAdvancePaymentRemarks(rep?.remarks);
        const penaltyExempt = isPenaltyExemptByPlanPosition({
          localPhaseNumber: phaseNumber,
        });
        let displayStatus = computeStatus(
          rep.status,
          rep.due_date,
          rep.issue_date,
          penaltyExempt
        );
        if (remainingBalance <= 0.009 && paidAmount >= 0.009) {
          displayStatus = 'Paid';
        } else if (paidAmount > 0.009 && remainingBalance > 0.009) {
        ...
        return {
          ...
          invoice_status: rep.status || null,
          is_generated: true,
      ...
      for (let localPhase = 1; localPhase <= maxPhaseRows; localPhase += 1) {
        phases.push(await buildPhaseRow(localPhase, chainByLocalPhase.get(localPhase) || null));
      }
      ...
      const outstandingGenerated = normalizedPhases.reduce((sum, p) => {
        ...
        if (p.remaining_balance != null) {
          return sum + Math.max(0, Number(p.remaining_balance));
        }
      ...
      res.json({
        ...
        data: {
          profile: {
            ...
            lifecycle_is_active: resolveInstallmentPlanLifecycleActive({
              isActive: profile.is_active === true,
              upgradedToFullPayment: fullPaymentConversion?.upgraded === true,
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (isLastInvoice) {
        ...
        const nextGenYmd =
          nextPhaseSchedule?.current_generation_date ||
          (next_generation_date ? String(next_generation_date).trim().slice(0, 10) : null);
        const nextMonthYmd =
          nextPhaseSchedule?.current_invoice_month ||
          (next_invoice_month ? String(next_invoice_month).trim().slice(0, 10) : null);
        const fallbackDate = todayYmdManila();
        await client.query(
          `UPDATE installmentinvoicestbl 
 ...
 *      that phase), so the scheduler targets the following phase — including for
 *      **partial** advance (so the next phase is not generated on the same 25th).
 *   5. Enrolls the student in classstudentstbl for the absolute phase only when
 *      the phase chain is **fully settled** (partial advance keeps Enrollment blank
 *      so Month Re-enrollment / Total Active / Student Status stay Inactive).
 *   6. Due date follows the installment cadence (queue anchor), not payment_date +
 *      (generatedCount-1) month shift.
 *
...
router.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      let issueDateYmd = payment_date
        ? String(payment_date).slice(0, 10)
        : todayYmdManila();
      let dueDateYmd = null;

      if (isPhaseInstallmentProfile(profile)) {
        // Pass the stored generation queue as the recurring anchor. Using only
        // payment_date as issueDateOverride (without next_generation_date) re-seeds
        // from the payment day and advances by generatedCount-1 — e.g. Phase 5
        // advance on Jul 9 wrongly due Nov 5 instead of Aug 5.
        const phaseSchedule = await buildPhaseInstallmentSchedule({
          ...
          // Override issue (when paid early) but keep due on the cadence cycle.
          issueDateOverride: payment_date ? String(payment_date).slice(0, 10) : todayYmdManila(),
        });
        dueDateYmd = phaseSchedule?.current_due_date || null;
        if (!payment_date && phaseSchedule?.current_issue_date) {
      ...
      const normalizedPaymentType = String(payment_type || 'Advance Payment').trim();
      const isPartialAdvance = normalizedPaymentType === 'Partial Payment';
      const grossPayable = isPartialAdvance
      ...
      if (isPartialAdvance) {
        if (!Number.isFinite(grossPayable) || grossPayable <= 0) {
          ...
          return res.status(400).json({
            success: false,
            message: 'Payable amount is required for partial advance payment.',
          });
        }
        if (invoiceAmount > 0 && grossPayable >= invoiceAmount) {
          await client.query('ROLLBACK');
          return res.status(400).json({
            success: false,
            message: 'For partial payment, amount must be less than the phase payable amount.',
          });
        }
      }
      ...
      const recordedPaymentType = isPartialAdvance ? 'Partial Payment' : 'Full';
      await client.query(
      ...
      let balanceInvoiceId = null;
      if (isPartialAdvance) {
        const totalSettled = netPayable + discountValue;
        const remainingBalance = Math.max(0, invoiceAmount - totalSettled);
        if (remainingBalance > 0.01) {
          const { balance_invoice_id } = await createBalanceInvoiceFromPartial({
            client,
            parentInvoice: newInvoice,
            remainingBalance,
            createdBy: creatorUserId,
            issueDateYmd,
          });
          balanceInvoiceId = balance_invoice_id;
        } else {
      ...
      await syncProgramPaymentStatusForInvoice(client, newInvoice.invoice_id);
      if (balanceInvoiceId) {
        await syncProgramPaymentStatusForInvoice(client, balanceInvoiceId);
      }

      // Enroll only when the phase chain is fully settled (partial advance waits).
      if (profile.class_id && profile.student_id) {
        ...
        if (parentInvoiceRes.rows[0]) {
          await syncInstallmentEnrollmentForPaidInvoice({
            client,
            profileId: parseInt(id, 10),
            profile,
            studentId: profile.student_id,
            sourceLabel: isPartialAdvance
              ? 'System (Auto-enrolled via partial advance payment)'
              : 'System (Auto-enrolled via advance payment)',
            invoice: parentInvoiceRes.rows[0],
          });
        }
      }

      // ---- Advance the auto-generation schedule (full AND partial advance) ----
      // Partial advance must still consume the generation slot so the next phase
      // is not auto-generated on the same 25th (e.g. Phase 5 partial on Jul 9
      // must push Phase 6 to Aug 25, not generate it on Jul 25).
      ...
      // Full advance: ensure class enrollment row exists (sync above also handles settle).
      // Partial advance: enrollment stays blank until the balance chain is Paid.
      if (!isPartialAdvance && profile.class_id && profile.student_id) {
      ...
      if (isPartialAdvance) {
        const receiptInvoiceId = balanceInvoiceId || newInvoice.invoice_id;
        return res.json({
          success: true,
          message: `Partial advance payment recorded for Phase ${absolutePhaseNumber}.`,
          data: {
            invoice_id: receiptInvoiceId,
            parent_invoice_id: newInvoice.invoice_id,
