 ...
 * - Older duplicate plan for same student+class (superseded): use newest profile only
 */
export function canGenerateInstallmentInvoice(row) {
  if (!row) return false;
  if (row.plan_superseded === true) return false;
...
/** @returns {'unenrolled' | 'complete' | 'superseded' | null} */
export function installmentGenerateBlockedReason(row) {
  if (!row) return 'unenrolled';
  if (row.plan_superseded === true) return 'superseded';
