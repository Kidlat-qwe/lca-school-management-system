  ...
  | File | Purpose |
  | ---- | ------- |
  | `InstallmentPlanDetails.jsx`         | Fetches and renders one installment plan (plan card, optional downpayment card, phases table, totals). Supports **Pay Now** on the earliest actionable phase: existing unpaid invoice via `POST /payments`, or advance pay via `POST .../advance-pay`. **Rejoin** appears only when the latest actionable state is an **unpaid dropped** phase with **no continue** afterward. If the student already rejoined/continued (later paid or active phase), Rejoin is hidden and **Pay Now** is offered on the next phase after the latest continued invoice (e.g. Phase 7 after paid Phase 6 rejoin). Intermediate phases between a prior drop and a later rejoin stay **visible** as `-` / Not Generated (not hidden as `late_start_gap`); in the **Enrollment** column those same intermediate gap phases show **"Not Enrolled"** instead of a blank dash, since they occurred in the past (before the latest phase with real billing/enrollment activity). Future not-yet-reached phases keep a blank Enrollment cell — their enrollment outcome isn't known yet. True late-start gaps (never enrolled before first billable phase) remain hidden. Before opening payment, calls `GET /invoices/:id` and blocks with an alert if an earlier phase has unsettled partial-payment balance (`prior_partial_balance_block`). Reused inline by other dialogs. |
