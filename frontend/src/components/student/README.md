...
- **Installment tab:** Renders one **inline plan-details panel per installment plan** (see `components/installmentInvoice/InstallmentPlanDetails.jsx`) with the same layout as the **View details** dialog on the Installment Invoice Logs page — student/plan card (including **class started date**), optional downpayment card, full phases table (paid, unpaid, not yet generated), and totals (outstanding balance + total paid). Empty state points to the Full payment tab when the student has no installment plans.
- **Full payment tab:** One settlement card per native full-payment invoice or installment→full-payment conversion (`GET /invoices/student/:id/full-payment`). Shows package/class/**class started date**/phase coverage, **student Active/Inactive**, invoice + line items (conversion credits as discounts), payments, enrollment chips (**Phase 1 = new** on this class track; later phases re-enrolled; per-phase start date from class sessions), merchandise, and Invoice/AR PDF. Full payment is **not** mixed into the installment 10-row phase table. Zero-balance conversions without a conversion invoice remain on the Installment tab (`Inactive · Upgraded to Full Payment`).
...
- **Re-enrollment matrix:** Clicking a student name opens this dialog on **Full payment** when the matrix row is a full-payment (or converted) track, otherwise **Installment**. See `utils/enrollmentMatrixStudentHistory` (`initial_tab` / `is_full_payment`).
- **Props:**
  - `isOpen`, `student`, `onClose`
  - `student.initial_tab` — `'full-payment'` or `'invoices'` from the matrix click
  - `student.focus_class_id` / `focus_class_name` — highlights the matching class settlement/plan
