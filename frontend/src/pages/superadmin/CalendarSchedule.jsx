...
const CalendarSchedule = () => {
  ...
  }, [viewMode, selectedMonth, selectedWeekStart, teacherFilter, branchFilter, roomFilter]);

  // Clear room and teacher filters when branch changes
  useEffect(() => {
    if (!branchFilter) {
      setRoomFilter('');
      setTeacherFilter('');
  ...
  const fetchSchedules = async () => {
    ...
    try {
      ...
      if (branchFilter && branchFilter !== '') params.append('branch_id', branchFilter);
  ...
  const handleGoToClassPage = () => {
    ...
    if (classId) {
      navigate(`/superadmin/classes?classId=${classId}`);
    } else {
              ...
              {loadingClassDetails ? (
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="text-xs font-semibold uppercase tracking-wide text-gray-500">Branch</label>
                      <p className="mt-1 text-sm text-gray-900">{classDetails.branch_name || selectedEvent?.branch_name || 'N/A'}</p>
                    </div>
                    <div>
                      <label className="text-xs font-semibold uppercase tracking-wide text-gray-500">Room</label>
